...
{
	Bling_Set(&Light_3,500,50,0.1,0,GPIO_PORT_P2,GPIO_PIN1,0);//红色  //用户移植时，重写此函数
}
...
void NCLink_SEND_StateMachine(void)
{
	...
	if(!NCLink_Send_Check_Status_Parameter())//判断地面站有无请求数据、是否需要发送应答
	{
		...
		{
			NCLink_Cnt=9;
			NCLink_Send_Userdata(PPM_Databuf[0],PPM_Databuf[1],PPM_Databuf[2],PPM_Databuf[3],PPM_Databuf[4],PPM_Databuf[5]);		
		}
