  ...
  if(IMU_ID==IMU_MPU6050_ID)//MPU6050
	{
		...
		Single_WriteI2C(MPU_ADRESS,MPU_CONFIG  , 0x00); //内部低通滤波频率，加速度计94hz，陀螺仪98hz
//		Single_WriteI2C(MPU_ADRESS,MPU_CONFIG  , 0x03);//内部低通滤波频率，加速度计44hz，陀螺仪42hz
	...
	{	
		Single_WriteI2C(MPU_ADRESS,PWR_MGMT_1  , 0x00);//关闭所有中断,解除休眠
		Single_WriteI2C(MPU_ADRESS,SMPLRT_DIV  , 0x00); // sample rate.  Fsample= 1Khz/(<this value>+1) = 1000Hz	
		Single_WriteI2C(MPU_ADRESS,MPU_CONFIG  , 0x00);//设置陀螺仪、温度内部低通滤波频率范围，陀螺仪41hz，噪声带宽59hz，温度42hz
//		Single_WriteI2C(MPU_ADRESS,MPU_CONFIG  , 0x03);//设置陀螺仪、温度内部低通滤波频率范围，陀螺仪41hz，噪声带宽59hz，温度42hz
		Single_WriteI2C(MPU_ADRESS,GYRO_CONFIG , 0x10);//设置陀螺仪量程，1000deg/s
		Single_WriteI2C(MPU_ADRESS,ACCEL_CONFIG, 0x18);// Accel scale 16g (2048 LSB/g)	
		Single_WriteI2C(MPU_ADRESS,ACCEL_CONFIG2,0x03);//设置加速度计内部低通滤波频率范围，加速度44.8hz，噪声带宽61.5hz
	}
  delay_ms(500);
  IMU_Calibration();
