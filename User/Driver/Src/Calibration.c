...
void Accel_Calibration_Check(void)
{
	       ...
	       &&Controler_State==Lock_Controler)//必须为上锁状态才可以进入校准模式
  {
    ...
    Cal_Flag=0;	
		Bling_Set(&Light_2,1000,100,0.5,0,GPIO_PORT_P2,GPIO_PIN0,1);
    flight_direction=6;
     ...
     &&Accel_Calibration_All_Finished==0)//6面全部校准完毕
  {
    ...
    if(Cal_Flag==TRUE)//加速度计校准成功
    {
      ...
      Bling_Mode=0;//恢复正常指示模式
			Bling_Set(&Light_2,3000,1000,0.5,0,GPIO_PORT_P2,GPIO_PIN0,0);
			NCLink_Send_Check_Flag[9]=0x01;//校准完毕后发送校准数据
    }
    else//加速度计校准失败
    {
      Bling_Mode=0;//恢复正常指示模式
			Bling_Set(&Light_2,5000,200,0.5,0,GPIO_PORT_P2,GPIO_PIN0,0);
...
void Mag_Calibration_Check(void)
{
    ...
    //进入磁力计校准模式
  {
    ...
    Mag_Calibration_Mode=3;
		Bling_Set(&Light_2,1000,500,0.5,0,GPIO_PORT_P2,GPIO_PIN0,1);
...
uint8_t Mag_Calibartion(Vector3f *magdata,Vector3f_Body Circle_Angle_Calibartion)
{
       ...
       &&Mag_Is_Okay_Flag[2]==1)//三面数据全部采集完毕，计算磁力计零点
  {
    ...
    Mag_Offset[2]=(int16_t)(Mag_Offset_Read.z);
		...
		Bling_Set(&Light_2,5000,1000,0.5,0,GPIO_PORT_P2,GPIO_PIN0,0);   
    //写入加速度零点偏执与磁力计中心偏执 
...
void ESC_Calibration_Check(void)
{
    ...
    //进入ESC校准模式
  {
    ...
    WriteFlashParameter(ESC_CALIBRATION_FLAG,
                        ...
                        &Table_Parameter);
    Bling_Set(&Light_2,5000,500,0.2,0,GPIO_PORT_P2,GPIO_PIN0,1);
    Page_Number=14;
  }
}
...
void Horizontal_Calibration_Check(void)
{
             ...
             &&Controler_State==Lock_Controler)//必须为上锁状态才可以进入校准模式
  {
    ...
    Bling_Set(&Light_2,500,100,0.2,0,GPIO_PORT_P2,GPIO_PIN0,0);
...
uint8_t Gyro_Calibration_Check(vector3f *gyro)
{
  ...
  static vector3f last_gyro;
   if(gyro->x-last_gyro.x<=500
		   &&gyro->y-last_gyro.y<=500
	      &&gyro->z-last_gyro.z<=500
  ...
  last_gyro.x=gyro->x;
    ...
    Bling_Set(&Light_2,2000,100,0.2,0,GPIO_PORT_P2,GPIO_PIN0,0);
		NCLink_Send_Check_Flag[9]=0x01;
	  return 1;
	}
	return 0;
